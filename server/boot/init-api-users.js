...
module.exports = function (app, done) {
  ...
  var defaultUsers = [
    { username: 'admin', email: 'admin@acme.com', password: process.env.INIT_ADMIN_PASSWORD, roles: [ 'admin' ] },
    { username: 'guest', email: 'guest@acme.com', password: process.env.INIT_GUEST_PASSWORD }
  ]
          ...
          if (err || !existing) {
            await createUser(user).then(created => {
              if (created.username === 'admin') {
                process.env.ADMIN_ID = created.id
              }
              resolve(created)
            })
          } else {
            if (existing.username === 'admin') {
              process.env.ADMIN_ID = existing.id
            }
            resolve(existing)
          }
        })
