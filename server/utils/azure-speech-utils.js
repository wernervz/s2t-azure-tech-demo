...
AzureSpeechUtilsLocal.prototype.submitBatchTranscription = async function(fileName) {
  console.log("In submit Batch transcription for file " + fileName);
  ...
  let containerSAS = AzureBlobUtilsInternal.getContainerSAS(
  ...
  try {
    ...
    let triggerResponse = await got.post(
      "https://eastus.cris.ai/api/speechtotext/v2.1/transcriptions",
      options
    );

    return {
      success: triggerResponse.statusCode < 300,
      reference: triggerResponse.headers.location
    };
  } catch (err) {
      console.log('Error Submitting transcription!!!')
      console.log(err)
      throw err
  }
};
